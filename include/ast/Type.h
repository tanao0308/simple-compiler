...
struct Variable {
    ...
    Variable operator+(const Variable &rhs) const { return fromDouble(asNumber() + rhs.asNumber()); }
    Variable operator-(const Variable &rhs) const { return fromDouble(asNumber() - rhs.asNumber()); }
    Variable operator*(const Variable &rhs) const { return fromDouble(asNumber() * rhs.asNumber()); }
    Variable operator/(const Variable &rhs) const { return fromDouble(asNumber() / rhs.asNumber()); }
    Variable operator^(const Variable &rhs) const { return fromDouble(pow(asNumber(), rhs.asNumber())); }
...
struct Function {
    ...
    Function(std::string name, std::string param, std::shared_ptr<Stmt> body) : name(name), param(param), body(body) {}
};